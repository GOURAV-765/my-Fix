...
interface AuthContextType {
 ...
 // Used by the login page to avoid rendering <SignIn>, which would auto-errorirect
...
// PrivateRoute cannot bounce the user to /login where Clerk would auto-errorirect
...
const AuthBlockedScreen: React.FC<{ error: string | null }> = ({ error }) => (
 ...
 <div>
 <h1 className="text-xl font-bold text-textPrimary">Session found, account not ready</h1>
 <p className="text-sm text-textMuted mt-2">
 You are signed in, but your society portal account could not be loaded.
 </p>
 </div>
 {error && (
 <p className="text-xs text-error bg-error/20 border border-error/20 rounded-lg p-3">
 {error}
 </p>
 )}
 <div className="pt-2">
 <SignOutButton>
 <button className="w-full py-2.5 px-4 bg-ieeeBlue hover:bg-ieeeBlue text-white text-sm font-semibold rounded-xl transition-all duration-200 cursor-pointer">
