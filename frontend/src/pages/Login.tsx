...
const LocalLoginForm: React.FC = () => {
 ...
 {error && (
 <div className="p-3 text-xs bg-error/40 border border-error/20 text-error rounded-xl text-center">
 {error}
 </div>
 )}

 <div className="space-y-1.5">
 <label className="text-xs font-semibold text-textBody">Email Address</label>
 ...
 <div className="space-y-1.5">
 <label className="text-xs font-semibold text-textBody">Password</label>
 <div className="relative">
 ...
 type="submit"
 disabled={loading}
 className="w-full py-2.5 px-4 bg-ieeeBlue hover:bg-ieeeBlue disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-ieeeBlue/10 flex items-center justify-center gap-2 cursor-pointer"
 >
 {loading ? (
...
const Login: React.FC = () => {
 ...
 // immediately auto-errorirect back to /dashboard, recreate the reload loop. Show a stable
 ...
 <p className="text-sm text-textMuted mt-2">{profileError}</p>
 </div>
 ...
 <div>
 <h1 className="text-2xl font-bold text-textPrimary leading-tight">Society Management</h1>
 <p className="text-sm text-textMuted mt-1">Sign in to manage your tenant portal</p>
 </div>
 </div>
