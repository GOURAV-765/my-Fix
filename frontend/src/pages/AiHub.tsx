...
const AiHub: React.FC = () => {
 ...
 <div>
 <label className="block text-textMuted mb-1">Biography / Career Focus</label>
 <textarea
 required
 {...resumeReg('bio')}
 ...
 <div>
 <label className="block text-textMuted mb-1">List Skills</label>
 <input
 type="text"
 required
 {...resumeReg('skills')}
 ...
 <div>
 <label className="block text-textMuted mb-1">Target Languages & Frameworks</label>
 <input
 type="text"
 required
 {...resumeReg('techStack')}
 ...
 <h3 className="font-extrabold text-textPrimary text-sm flex items-center gap-2">
 <Cpu className="h-5 w-5 text-ieeeBlue" />
 ...
 <span className="h-1.5 w-1.5 bg-error rounded-full shrink-0" />
 {gap}
 </div>
 ))}
 </div>
 ...
 <div className="space-y-3 text-xs text-textBody">
 {resumeResult.interviewQuestions.map((q, idx) => (
 <div key={idx} className="bg-appBg/40 p-3 rounded-xl border border-slate-850 flex gap-2">
 <span className="font-bold text-ieeeBlue font-mono">Q{idx + 1}:</span>
 <p>{q}</p>
 </div>
 ))}
 </div>
 ...
 <p className="text-xs text-textMuted leading-normal">{resumeResult.recommendations}</p>
 </div>
 </div>
 )}
 </div>
