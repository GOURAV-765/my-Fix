...
const MemberProfile: React.FC = () => {
 ...
 if (isLoading) {
 return (
 <div className="flex h-64 items-center justify-center text-textMuted animate-pulse">
 <div className="flex flex-col items-center gap-3">
 <div className="h-8 w-8 animate-spin rounded-full border-4 border-ieeeBlue border-t-transparent"></div>
 ...
 if (isError || !member) {
 return (
 <div className="p-6 text-center text-error space-y-4">
 ...
 <Link to="/members" className="inline-block px-4 py-2 bg-slate-100 hover:bg-slate-700 text-textPrimary text-xs font-semibold rounded-lg">
 Back to Members
 </Link>
 </div>
 );
 ...
 className="flex items-center gap-2 px-4 py-2 bg-warning hover:bg-warning text-white rounded-xl text-xs font-semibold shadow-lg shadow-warning/10 transition-all"
 >
 <Edit2 size={12} />
 Edit Profile
 </Link>
 ...
 className="h-28 w-28 rounded-full object-cover border-2 border-ieeeBlue/20"
 />
 ) : (
 <div className="h-28 w-28 rounded-full bg-ieeeBlue flex items-center justify-center font-bold text-4xl text-ieeeBlue border-2 border-ieeeBlue/20">
 {member.firstName[0]}
 </div>
 )}
 <span
 ...
 <div className="p-2 rounded-xl bg-cardBg border border-slate-850 text-ieeeBlue shrink-0">
 <Mail size={16} />
 </div>
